...
Folio is multi-tenant: each Workshop is an isolated environment with its own members, works, and critique rounds. We must decide how to isolate tenant data in PostgreSQL.

The two main strategies are:

1. **Row-level tenancy:** All tenants share the same tables. Every row has a `workshop_id` foreign key. Queries filter by `workshop_id`.
2. **Schema-per-tenant:** Each tenant gets its own PostgreSQL schema (namespace). Tables are duplicated per schema.

## Decision

Use **row-level tenancy** with `workshop_id` on all relevant tables.
...
- **Query enforcement.** The application layer enforces tenant isolation: every service function receives the `workshopId` from the authenticated user's context and passes it to every query. This is verified in integration tests.

## Trade-offs

- **Accidental data leakage risk.** If a service function forgets to filter by `workshop_id`, it could return rows from other workshops. Mitigated by: (1) keeping `workshopId` in the tRPC context and passing it explicitly, (2) integration tests that assert cross-tenant isolation, (3) PostgreSQL Row Level Security (RLS) as a future hardening step.
